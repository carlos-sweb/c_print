...
///
/// Demonstrates the pattern-based API (z_print_mod.z_print).
...
pub fn main() !void {
    ...
    // Simple colored text
    try cp.z_print(&writer, "Hello {s:green}!\n", .{"World"});

    // Multiple specifiers combined
    try cp.z_print(&writer, "{s:cyan:bg_black:bold}\n", .{"IMPORTANT"});
    ...
    try cp.z_print(&writer, "Population: {d:,}\n", .{@as(i32, 1234567)});

    // Percentage formatting
    try cp.z_print(&writer, "Progress: {f:.2:%:cyan}\n", .{@as(f64, 0.85)});

    // Hexadecimal with prefix
    try cp.z_print(&writer, "Hex: {x:#:bold}\n", .{@as(u64, 255)});

    // Price with currency and separators
    try cp.z_print(&writer, "Price: ${f:.2:,}\n", .{@as(f64, 1234.56)});

    // Alignment examples
    try cp.z_print(&writer, "|{s:<20}|\n", .{"Left"});
    try cp.z_print(&writer, "|{s:>20}|\n", .{"Right"});
    try cp.z_print(&writer, "|{s:^20}|\n", .{"Center"});
    try cp.z_print(&writer, "|{s:*^20}|\n", .{"Fill"});

    // Binary and octal
    try cp.z_print(&writer, "Binary: {b:#}\n", .{@as(u64, 42)});
    try cp.z_print(&writer, "Octal:  {o:#}\n", .{@as(u64, 42)});

    // Integer with zero padding
    try cp.z_print(&writer, "Padded: {d:05}\n", .{@as(i32, 42)});

    // Integer with sign
    try cp.z_print(&writer, "Positive: {d:+}\n", .{@as(i32, 42)});
    try cp.z_print(&writer, "Negative: {d:+}\n", .{@as(i32, -42)});

    // Long integer with separator
    try cp.z_print(&writer, "Big number: {l:,}\n", .{@as(i64, 1234567890)});
